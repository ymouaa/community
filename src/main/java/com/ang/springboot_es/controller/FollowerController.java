...
public class FollowerController implements DemoConstant {
	...
	public String follow(int entityType, int entityId) {
		...
		String redisKey = RedisKeyUtil.getTempKey(user.getId(), entityId, entityType, TOPIC_FOLLOW);
		if (redisTemplate.opsForValue().setIfAbsent(redisKey, 1, 60, TimeUnit.SECONDS)) {
			// 触发关注事件
			Event event = new Event().setTopic(TOPIC_FOLLOW).setUserId(user.getId()).setEntityType(entityType)
